...
void EulerSpiral::computeSpiral(vcl_vector<Point2D<double> > &spiral, double ds, int NPts)
{
  if (ds==0 && NPts==0){
    //use default values
    NPts = 100;
...
Point2D<double> EulerSpiral::compute_end_pt(double k0, double gamma, double L, bool bNormalized)
{
  ...
  double C = sign(gamma)*(fresnel1.getX() - fresnel2.getX());
...
Point2D<double> EulerSpiral::get_fresnel_integral(double x)
{
  ...
  { 
    ...
    if (ax <= XMIN)
    {
      ...
      if (k > MAXIT) 
        vcl_cout << "series failed in fresnel" << vcl_endl;
      ...
      if (k > MAXIT) 
        vcl_cout << "cf failed in frenel" << vcl_endl;
...
// get Euler Spiral parameters

void EulerSpiral::getParams(double *eulerParams) {

	eulerParams[0] = params.K0;
	eulerParams[1] = params.gamma;
	eulerParams[2] = params.L;
